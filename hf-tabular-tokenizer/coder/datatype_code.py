...
from utils.functions import inverse_log1p

logger = create_logger(__name__)


__all__ = ["IntCode", "FloatCode", "CategoryCode", "BinnedCategoryCode"]
    ...
    def get_best_transform(self, data_series: ndarray) -> str:
        ...
        for transform in self.AVAILABLE_TRANSFORMS:
            ...
            try:
                ...
                logger.error(f'{self.name} with {self.transform} and code len {self.code_len} gives a {exc}')
                continue
...
class BinnedCategoryCode(CategoryCode):
    """For Floats that have been binned as categories. Helper class so you can reuse the same token IDs"""
    def __init__(self, col_name: str, start_id: int):
        super().__init__(col_name, start_id)
