...
def find_index_of(list_input, item) -> int:
    output = -1
    try:
...
def is_list_of_ints(obj: list) -> bool:
    """
    Check if object for decoding is a single list (list of ints) or a batch of lists (a list of lists of ints).
    """
    if isinstance(obj, list):
        if all(isinstance(item, int) for item in obj):
            return True
        elif all(isinstance(item, list) and all(isinstance(subitem, int) for subitem in item) for item in obj):
            return False
    return False
...
class TabularTokenizer(TokenizerSpec):
    ...
    def __init__(self, coder, special_tokens=None, delimiter=','):
        ...
        for column in self.code_column.columns:
            if hasattr(self.code_column.column_codes[column],
                       'special_tokens_to_token_ids') and self.partial_row_placeholder in self.code_column.column_codes[
                column].special_tokens_to_token_ids:
    ...
    def text_to_tokens(self, text: str, columns: Optional[List[str]] = None) -> List[int]:
        ...
        logger.debug("token column width : ", token_column_width)
        ...
        logger.debug("token_ids shape : ", token_ids.shape)
        current_col_idx = 0
        ...
        for idx, column_name in enumerate(columns):
            subset = data[:, idx]
            tokens = []
            if self.code_column.column_codes[column_name].__class__.__name__ in ['CategoryCode', 'BinnedCategoryCode']:
                subset = subset.astype(str)
                for item in subset:
                    if item == self.partial_row_placeholder:
                        tokens.append(self.code_column.column_codes[column_name].code_len * [
    ...
    def encode(self, text: str, columns: Optional[List[str]] = None):
        ...
        if text == '':
            return []
        elif text == END_OF_TEXT:
            return self.special_tokens_encoder[END_OF_TEXT]
    ...
    def token_ids_to_text(self, token_ids: List[int]) -> str:
        ...
        decoded = []
        buffer = []
        dec_col_idx = -1  # decoder column index
    ...
    def decode(self, token_ids: List[int]) -> Union[str, List[str]]:
        if is_list_of_ints(token_ids):
            return self.token_ids_to_text(token_ids)
        else:
            return [self.token_ids_to_text(batch) for batch in token_ids]

    def detokenize(self, token_ids: List[int]) -> Union[str, List[str]]:
        return self.decode(token_ids)
